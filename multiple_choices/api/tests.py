...
class MultipleChoiceAnswerTests(APITestCase):
    ...
    def test_multiple_choice_can_be_answered(self):
        response = self.client.post(
            f"/api/v1/users/{self.user.id}/multiple-choices/{self.multiple_choice.id}/answers/",
            self.data, format="json")
        print(response.json())
        self.assertEqual(response.status_code, 201)


    def test_Should_NotAcceptAnswersOfOtherUsersOrMultipleChoices(self):
        self.client.post(f"/api/v1/users/{self.user.id}/multiple-choices/{self.multiple_choice.id}/answers/",
                         self.data, format="json")
        data = {
            "multiplechoicesolutionanswer_set": [
        ...
        self.client.post(f"/api/v1/users/2/multiple-choices/3/answers/", data, format="json")
        response = self.client.get(f"/api/v1/users/{self.user.id}/multiple-choices/3/answers/")
        for answer_json in response.json():
            self.assertEqual(answer_json.get("user"), self.user.id)
